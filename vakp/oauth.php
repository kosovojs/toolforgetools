...
$conn = connect_db("s53143__wmlCheck_p");
...
switch ( isset( $_REQUEST['act'] ) ? $_REQUEST['act'] : '' ){
	...
	case "ok":
		header('Content-Type: application/json');
		do_sql_query('ok',$_REQUEST['data']);
		break;
		
	case "no":
		header('Content-Type: application/json');
		do_sql_queryNO('no',$_REQUEST['data']);
		break;
}
...
function getdata() {
	$comm = "SELECT item,itemLabel,adrese,norades,dalaNo, saraksts,p131 FROM objekti WHERE ((no is null and checked is null and wd_complex is null)) ORDER BY RAND() limit 1";
	$result = do_sql($comm);
	$numofres = "SELECT count(*) FROM objekti WHERE ((no is null and checked is null and wd_complex is null))";
	$result1 = do_sql($numofres);
	
	echo json_encode(array($result,$result1) );
}


function do_sql_query($action,$dataOK) {
	date_default_timezone_set('Europe/Riga');
	$cur_time = date("YmdHis");
	global $conn;
    isAuthOkay();
	//$data = json_decode($dataOK, true);
	$data = $dataOK;
	
	//var_dump($data);
	
	//{'currWDitem':currWDitem,'coords':coords,'ref':ref,'wd':wd,'notes':notes,'isComplex':isComplex}
	...
	if (array_key_exists("coords",$data)) {
		$curr = test_input($data['coords']);
		if ($curr!='')
			$sqlstatement[] = "coords = '".$curr."'";
	}
	
	if (array_key_exists("ref",$data)) {
		$curr = test_input($data['ref']);
		if ($curr!='')
			$sqlstatement[] = "ref = '".$curr."'";
	}
	
	if (array_key_exists("wd",$data)) {
		$curr = test_input($data['wd']);
		if ($curr!='')
			$sqlstatement[] = "wd_obj = '".$curr."'";
	}
	
	if (array_key_exists("notes",$data)) {
		$curr = test_input($data['notes']);
		if ($curr!='')
			$sqlstatement[] = "notes = '".$curr."'";
	}
	
	if (count($sqlstatement)<1) {return; }
	
	//var_dump($sqlstatement);
	//var_dump(count($sqlstatement));
	
	if (array_key_exists("isComplex",$data)) {
		$curr = test_input($data['isComplex']);
		
		if ($curr == '1') {
			$wditem = test_input($data['currWDitem']);
			$comm = "SELECT norades, saraksts FROM objekti WHERE item='$wditem'";
			$resusdfsdfsdflt = do_sql($comm);
			$norade = $resusdfsdfsdflt[0]['norades'];
			$sarakstys = $resusdfsdfsdflt[0]['saraksts'];
			$sqlqueryfsdfdfd = "UPDATE objekti set author='$username', checked=$cur_time, isCompl='yes' where item='$wditem'";
			$resulfsdfdt = mysqli_query($conn,$sqlqueryfsdfdfd) or die(mysqli_error($conn));
			$gfdgdfgfgdfgdfgdfgdfgdfgd = "UPDATE objekti set wd_complex='yes', ".implode(', ',$sqlstatement)." where norades='$norade' and saraksts='$sarakstys'";
			$resusdfdfsdfsdfsdfsdfsdlt = mysqli_query($conn,$gfdgdfgfgdfgdfgdfgdfgdfgd) or die(mysqli_error($conn));
	...
	$sqlquery = "UPDATE objekti set author='$username', checked=$cur_time, ".implode(', ',$sqlstatement)." where item='".test_input($data['currWDitem'])."'";
...
function do_sql_queryNO($action,$dataOK) {
	...
	$sqlquery = "UPDATE objekti set author='$username', checked=$cur_time, no='y' where item='".test_input($dataOK)."'";
